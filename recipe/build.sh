...
if [[ "${target_platform}" != "linux-"* ]] && [[ "${target_platform}" != "osx-"* ]]; then
  sed -i -E "s#(=)[^\|]*rattler-build_ocaml_[^\|]*Library#\1${_PREFIX_}/Library#g" "${SRC_DIR}/Makefile.config"
  sed -i -E "s#(\|)[^\|]*rattler-build_ocaml_[^\|]*Library#\1${_PREFIX_}/Library#g" "${SRC_DIR}/src/ocamlbuild_config.ml"
  cat $OCAMLLIB/Makefile.config
  cat "${SRC_DIR}/Makefile.config"
  cat "${SRC_DIR}/src/ocamlbuild_config.ml"
fi
